...
void inpteam(team *a, int numplayers) {
  ...
  for (int i = 0; i < a->numplayers; i++) {
    ...
    a->players[i].frita_id=fricta_id();
  }
}
...
int inpfouls(game *a,int j) {
  ...
  a->teams[j].numfouls = fouls;
  for (int i = 0; i < a->teams[j].numfouls; i++) {
    printf("Enter the name of the fritacole\n");
    scanf("%s", a->teams[j].fouls[i].name);
  }
...
int inppoints(game *a,int j) {
  ...
  for (int i = 0; i < a->teams[j].numpoints; i++) {
    printf("Enter the name of the fritacole who scored: \n");
    scanf("%s", a->teams[j].points[i].name);
  }
...
float sumheight(game *a, int j) {
  float hsum = 0;
  for (int i = 0; i < a->teams[i].numplayers; i++) {
...
void verifyadvantage(game *a) {
  ...
  (team1height > team2height)   ? printf("team one has advantage\n")
  : (team1height < team2height) ? printf("team two has advantage\n")
                                : printf("both teams have equal advantage\n");
  ;
}
...
void output(game *a){
  ...
  for(int i=0;i<a->teams[0].numfouls;i++){
    printf("%s\n",a->teams[0].fouls[i].name);
  }
  printf("Team 2: %d\n",a->teams[1].numfouls);
  for(int i=0;i<a->teams[1].numfouls;i++){
    printf("%s\n",a->teams[1].fouls[i].name);
  }
  printf("The number of points scored by each team are: \n");
  printf("Team 1: %d\n",a->teams[0].numpoints);
  for(int i=0;i<a->teams[0].numpoints;i++){
    printf("%s\n",a->teams[0].points[i].name);
  }
  printf("Team 2: %d\n",a->teams[1].numpoints);
  for(int i=0;i<a->teams[1].numpoints;i++){
    printf("%s\n",a->teams[1].points[i].name);
  }
